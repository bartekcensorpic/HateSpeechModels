...
def process_dataset(tokenizer, X, y,BATCH_SIZE, max_padding_length:None):
    ...
    reviews_with_len = [[review, y[i], len(review)]
                        for i, review in enumerate(tokenized_train_X)]
    ...
    sorted_reviews_labels = [(pad_sentence(review_lab[0], max_padding_length), review_lab[1]) for review_lab in reviews_with_len]
...
def train_bert_for_tf2(train_X, train_y, test_X, test_y,save_folder_path,num_epochs):
    ...
    #todo encapsulate creating dataset and do the same to test data
    BATCH_SIZE = 32


    train_dataset, max_padding_length = process_dataset(tokenizer, train_X, train_y,BATCH_SIZE, max_padding_length=None)
    test_dataset, _ = process_dataset(tokenizer, test_X, test_y,BATCH_SIZE, max_padding_length=max_padding_length)
    ...
    callbacks = get_callbacks(
        best_model_checkpoint_path=os.path.join(save_folder_path,'bert_for_tf2_model.h5'),
    ...
    text_model.fit(train_dataset, epochs=NB_EPOCHS, callbacks=callbacks, validation_data=test_dataset)
